...
public:
  ...
  HDF5DataSet(const std::filesystem::path& hdf5_filepath)
      : // IIFE
        axes{[&hdf5_filepath]() {
          // Of all the other helper functions this is the one that checks for
          // file existence just because it happens to be called first.
        ...
        values{[&hdf5_filepath]() {
          ...
          result = std::vector<double>(
              static_cast<size_t>(
                  block_dataset.getSpace().getSimpleExtentNpoints()),
              0);
        ...
        strides{[this]() {
          ...
          // level `D - 1`
          std::partial_sum(
              result.crbegin(), result.crend(), result.rbegin(),
              std::multiplies());
          return result;
        }()} {}
  ...
  Real at(size_t index, Args... inner_indices) const {
    ...
    return values.at(GetOffset(index, inner_indices...));
  }
  /// @brief Returns the offset or flattened index at the given axis indices
  /// @details If fewer indices than axes are given, an offset is returned. If
  ///          the number of indices matches the number of axes, the
  ///          corresponding flattened index to an element is returned.
  template <typename... Args>
  size_t GetOffset(size_t index, Args... inner_indices) const {
    // compile time error if too many indices
    static_assert(1 + sizeof...(inner_indices) <= D);
