...
public:
  ...
  GetMajorant(const State& s) const noexcept override;
  ...
  void AppendSecondaries(
      std::vector<History> bank, const State& state) const noexcept override;
...
private:
  ...
  bool ReactionsModifyTotal(const State& s) const noexcept;
