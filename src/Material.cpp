...
Material::GetMicroscopicMajorant(const State& s) const noexcept {
  // TODO: Memoize this call for performance
  return std::accumulate(
      afracs.cbegin(), afracs.cend(), MicroscopicCrossSection{0},
      [&s](const auto& accumulated, const auto& nuclide_afrac_pair) {
        const auto& [nuclide, afrac] = nuclide_afrac_pair;
        return accumulated + afrac * nuclide->GetMajorant(s);
      });
}

MicroscopicCrossSection
Material::GetMicroscopicTotal(const State& s) const noexcept {
  // TODO: Memoize this call for performance
  return std::accumulate(
      afracs.cbegin(), afracs.cend(), MicroscopicCrossSection{0},
      [&s](const auto& accumulated, const auto& nuclide_afrac_pair) {
        const auto& [nuclide, afrac] = nuclide_afrac_pair;
        return accumulated + afrac * nuclide->GetTotal(s);
      });
}
